...
class Plus9k
  def initialize(token:, event_path:, comment: nil)
  ...
  def default_comment
    <<~COMMENT
      ...
      Sharing your support via emoji reactions on comments avoids that problem and helps us get a complete picture
      of everybody's opinion. Make sure to use a reaction next time to make sure your vote is not lost.
    COMMENT
  end
  ...
  def reply(repo, issue_id)
    puts "Adding comment: #{repo}/#{issue_id}"
    @client.add_comment(repo, issue_id, @comment)
  end
end
